...
export default function PosPage() {
  ...
  const productPanel = (
    <div className="pk-card pos-panel" id="pos-products-panel">
          ...
          <div className="pos-product-grid">
            ...
            {filtered.length === 0 && (
              <div className="pk-empty" style={{ gridColumn: '1 / -1' }}>
                <i className="bi bi-search" /><p>No products match.</p>
              </div>
            )}
          </div>
        )}
  ...
  const cartPanel = (
    <div className="pk-card pos-panel" id="pos-cart-panel">
      <div className="pk-card__head">
        <div className="pk-card__title">Cart</div>
        {cart.length > 0 && (
          <span className="pk-badge pk-badge--rose" style={{ marginLeft: 'auto' }}>{cartCount} items</span>
        )}
      </div>
      <div className="pk-card__body" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button className="pk-btn pk-btn--outline" onClick={handleLookup}><i className="bi bi-search" /></button>
        </div>
      ...
      <div className="pos-mobile-layout">
        ...
        <div className="pos-mobile-tabs">
          ...
          >
            ...
            Cart
            {cartCount > 0 && <span className="pos-mobile-cart-dot">{cartCount}</span>}
          </button>
        </div>
        ...
        {/* Floating "View Cart" button when products tab is active */}
        {mobileTab === 'products' && cartCount > 0 && (
          <button className="pos-float-cart" onClick={() => setMobileTab('cart')}>
            <i className="bi bi-cart3" />
            View Cart ({cartCount})
            <span style={{ marginLeft: 'auto', fontWeight: 800 }}>{symbol} {grandTotal.toLocaleString()}</span>
          </button>
        )}
      </div>
        ...
        <div className="pk-modal-bd" onClick={(e) => { if (e.target === e.currentTarget) setLastOrder(null); }}>
          <div className="pk-modal" style={{ maxWidth: 420 }}>
            ...
            <div className="pk-modal__foot">
              ...
              <button className="pk-btn pk-btn--dark" onClick={handlePrint}><i className="bi bi-printer" />Print Receipt</button>
            </div>
          </div>
        </div>
      )}
